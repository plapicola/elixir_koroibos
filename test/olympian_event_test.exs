defmodule Koroibos.OlympianEventTest do
  ...
  test "Valid OlympianEvent", %{olympian: olympian, event: event} do
    changeset =
      OlympianEvent.changeset(%OlympianEvent{}, %{event_id: event.id, olympian_id: olympian.id})
  ...
  test "Olympian must exist for record to be valid", %{event: event} do
    {:error, changeset} =
      OlympianEvent.changeset(%OlympianEvent{}, %{event_id: event.id, olympian_id: -1})
      |> Repo.insert()
  ...
  test "Event must exist for record to be valid", %{olympian: olympian} do
    {:error, changeset} =
      OlympianEvent.changeset(%OlympianEvent{}, %{event_id: -1, olympian_id: olympian.id})
      |> Repo.insert()
